...
class ClauseMetaData(BaseModel):
    ...
    section_header: Optional[str] = Field(
        default=None,
        description="Section of header in the source document"
    )
    references: List[str] = Field(
        default_factory=list, 
        description="List of legal or document references"
    )
    amends: List[str] = Field(
        default_factory=list, 
        description="List of clauses this one amends"
    )
    overrides: List[str] = Field(
        default_factory=list, 
        description="List of clauses this one overrides"
    )
    source: Optional[str] = Field(
        default=None, 
        description="Source document or system for this clause"
    )
    ...
    num_sentences: Optional[int] = Field(
        default=None, 
        description="Number of sentences in the clause"
    )
    entities: Optional[List[str]] = Field(
        default=None, 
        description="Named entities extracted from the clause"
    )
    clause_type: Optional[str] = Field(
        default=None, 
        description="Type of clause, e.g., obligation, prohibition, etc."
    )
...
class Clause(BaseModel):
    ...
    title: Optional[str] = Field(
        default=None, description="Clause title or heading"
    )
    text: str = Field(
